...
export const LeverageBottomSheet: React.FC<LeverageBottomSheetProps> = ({
  ...
  useEffect(() => {
    if (visible) {
      ...
      // Scroll to current leverage position after sheet is shown
      setTimeout(() => {
  ...
  const handleLeverageSelect = (leverage: string) => {
    ...
    if (scrollViewRef.current) {
      const leverageIndex = leverageOptions.findIndex(opt => opt === leverage);
      if (leverageIndex >= 0) {
        const itemWidth = 40;
        const scrollToX =
  ...
  const renderLeverageItem = (leverage: string) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.7}
      >
        {/* Always reserve space for text, but hide if selected */}
        ...
        {/* Show indicator line - white and taller when selected */}
        <View
          style={[
